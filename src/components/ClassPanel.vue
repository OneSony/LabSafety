<template>
  <div class="class-panel">
    ...
    <div class="class-header">
      <h2>{{ classItemCopy.name }}</h2>
      <p><strong>课程ID:</strong> {{ classItemCopy.id }}</p>
      <p><strong>上课时间:</strong> {{ classItemCopy.start_time }}</p>
    ...
    <el-card class="comment-card">
      ...
      <div class="comment-list">
        ...
        <div
          v-for="(comment, index) in classItemCopy.commentList"
          :key="index"
          class="comment-item"
        >
          <p>
...
export default {
  ...
  data() {
    return {
      classItemCopy: { ...this.classItem },
      newComment: "", // 存储新评论的内容
      teacher: "", // 存储教师信息
      location: "", // 存储课程地点
      course_id: this.classItem.id,
      userLookup: {}, // 缓存用户信息的对象
    };
  },
  mounted() {
    console.log("classItem in class panel:", this.classItem);
  ...
  methods: {
    ...
    // 获取教师信息
    getTeacher() {
      classAPI.getTeachers(this.course_id).then((response) => {
        if (response.success) {
    ...
    getComments() {
      classAPI.getComments(this.course_id).then((response) => {
        if (response.success) {
          ...
          this.classItemCopy.commentList = response.data;
    ...
    // 返回课程面板
    goBack() {
      this.$emit("go-back-to-course-panel");
    },
    ...
    submitComment() {
      ...
      // TODO
      this.getComments();
      console.log("reget:", this.classItemCopy.commentList);
    },
  },
};
</script>
